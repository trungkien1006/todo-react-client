function RightCover({ fold }) {
    return (
        <div className={`w-2/5 h-full shadow-(--shadow-book) inset-shadow-sm origin-left relative duration-2000 transform-3d ${fold ? "transform-(--fold-move-the-book-cover-)": ""} z-2`}>
            <div className={`w-full h-full flex justify-end bg-(--old-book-color) rounded-l-lg absolute top-0 left-0 transform-(--rotateY-back) backface-hidden`}>
                <div className="w-1/2 h-full py-[32px] px-[32px] flex flex-col justify-between items-end">
                    ...
                    <button className="cursor-pointer font-medium text-lg duration-300 hover:duration-300 hover:opacity-[.5]">
                        Exit!
                    </button>
                </div>
            </div>
